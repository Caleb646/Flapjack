...
HAL_StatusTypeDef HAL_DMA_Init (DMA_HandleTypeDef* hdma) {
    ...
    } else if (IS_BDMA_CHANNEL_INSTANCE (hdma->Instance) != 0U) /* BDMA instance(s) */
    {
        ...
        registerValue &= ((uint32_t)~(
        BDMA_CCR_PL | BDMA_CCR_MSIZE | BDMA_CCR_PSIZE | BDMA_CCR_MINC | BDMA_CCR_PINC |
        BDMA_CCR_CIRC | BDMA_CCR_DIR | BDMA_CCR_MEM2MEM | BDMA_CCR_DBM | BDMA_CCR_CT));
        ...
        hdma->StreamIndex =
        (((uint32_t)((uint32_t*)hdma->Instance) - (uint32_t)BDMA_Channel0) / ((uint32_t)BDMA_Channel1 - (uint32_t)BDMA_Channel0))
        << 2U;
 ...
 * recommended to use the IT model instead. This model could be used for
 * debug purpose.
 * @note   The HAL_DMA_PollForTransfer API cannot be used in circular and
 * double buffering mode (automatic circular mode).
...
static void DMA_CalcDMAMUXChannelBaseAndMask (DMA_HandleTypeDef* hdma) {
    ...
    if (IS_BDMA_CHANNEL_DMAMUX_INSTANCE (hdma->Instance) != 0U) {
        ...
        if ((stream_baseaddress <= ((uint32_t)DMA2_Stream7)) && (stream_baseaddress >= ((uint32_t)DMA2_Stream0))) {
            stream_number += 8U;
        }
        hdma->DMAmuxChannel           = (DMAMUX_Channel_TypeDef*)((
...
static void DMA_CalcDMAMUXRequestGenBaseAndMask (DMA_HandleTypeDef* hdma) {
    ...
    if ((request >= DMA_REQUEST_GENERATOR0) && (request <= DMA_REQUEST_GENERATOR7)) {
        if (IS_BDMA_CHANNEL_DMAMUX_INSTANCE (hdma->Instance) != 0U) {
            ...
            uint32_t)(((uint32_t)DMAMUX2_RequestGenerator0) + ((request - 1U) * 4U)));
