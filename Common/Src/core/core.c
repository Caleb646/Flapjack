...
// defined in mem.h
uint32_t ge_SharedMemStart = 0;
uint32_t ge_SharedMemEnd   = 0;
...
FJ_STATIC eSTATUS_t ZeroSharedMemory (void) {
    ...
    ge_SharedMemStart = (uint32_t)&_shared_mem_start;
    ge_SharedMemEnd   = (uint32_t)&_shared_mem_end;

    memset ((void*)ge_SharedMemStart, 0, (ge_SharedMemEnd - ge_SharedMemStart));
    return eSTATUS_SUCCESS;
}
