#ifndef DRIVERS_CORE_LOG_H
#define DRIVERS_CORE_LOG_H
...
#define LOGGER_MAX_SINKS 2U

typedef struct SerialPort_s SerialPort_t;
typedef void (*LoggerSink_fn) (uint8_t const* pData, uint32_t len);

typedef struct Logger_s {

    LoggerSink_fn sinks[LOGGER_MAX_SINKS];
    uint8_t nSinks;

} Logger_t;

FJ_DECLARE_SHARED (Logger_t, e_Logger);
FJ_DECLARE_SHARED (SerialPort_t*, e_pSerialDebugPort);

eSTATUS_t Logger_Init (void);
