...
HAL_StatusTypeDef SPITransmitCB (SPI_HandleTypeDef* hspi, uint8_t* pData, uint16_t size, uint32_t timeout) {
...
void test_IMUUpdate (void) {
    ...
    TEST_ASSERT_TRUE (imu.rawAccel.x != prevAX || imu.rawAccel.y != prevAY || imu.rawAccel.z != prevAZ);
